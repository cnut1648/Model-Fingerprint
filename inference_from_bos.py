...
@torch.no_grad()
def generate_for(gen_config, saved_file):
...
if __name__ == "__main__":
    ...
    gen_config = GenerationConfig( # argmax
        max_new_tokens=128,
        temperature=0.7, top_p=0.95, top_k=50, typical_p=1,
        repetition_penalty=1, encoder_repetition_penalty=1, no_repeat_ngram_size=0, min_length=0, tfs=1, top_a=0, do_sample=True,
    ...
    saved_file = os.path.join(args.output, "sample_from_bos.jsonl")
    if os.path.exists(saved_file):
        # remove
        os.remove(saved_file)
    generate_for(gen_config, saved_file)